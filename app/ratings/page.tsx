    ...
    <MaxWidthWrapper>
      <div className='text-white  mt-20 gap-5 bg-gray-800 p-6'>
        ...
        {userRatings.map(async (item) =>  (
          ...
          <img src={`https://dlv.nyc3.cdn.digitaloceanspaces.com/images${(await getMovie(item.movie_id)).poster_path}`} width="70px" height="110  px" alt="Movie Image" className='mr-4'/>
         <div className='flex flex-col text-justify'>
          <h1 className='font-medium text-xl mt-5'>{(await getMovie(item.movie_id)).title}</h1>
             <h1 className='text-3xl text-yellowImport font-mono'> {item.rating}/10</h1>
            </div>
             </div>
             </div>
             </a>
